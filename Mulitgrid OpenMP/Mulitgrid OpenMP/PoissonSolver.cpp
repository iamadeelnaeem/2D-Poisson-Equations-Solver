...
#pragma omp parallel for collapse(2)
    for (int i = 1; i < coarseSize - 1; ++i) {
        for (int j = 1; j < coarseSize - 1; ++j) {
            // Indices for fine grid points corresponding to the coarser grid point
            int i_fine = 2 * i;
            int j_fine = 2 * j;

            coarseGrid[i][j] = 0.25 * (grid[i_fine][j_fine] + grid[i_fine + 1][j_fine] +
                grid[i_fine][j_fine + 1] + grid[i_fine + 1][j_fine + 1]);
        }
    }
